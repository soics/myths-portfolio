  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pointer-events-none fixed inset-0 z-40" aria-hidden="true">
      ...
      <motion.div animate={{ x: [0, 3, -2, 1, 0] }} transition={{ duration: 0.3, repeat: 3 }}
        ...
        style={{ background: 'linear-gradient(90deg, rgba(0,229,255,0.3), transparent 30%, transparent 70%, rgba(124,58,237,0.3))' }} />
...
function Vignette() {
  return (
    <div className="pointer-events-none fixed inset-0 -z-10" aria-hidden="true"
      style={{ background: 'radial-gradient(ellipse 65% 55% at 50% 50%, transparent 35%, rgba(6,6,14,0.55) 100%)' }}
    />
  )
}
